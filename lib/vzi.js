...
const hour = (unix) => {
  ...
  return `${T.datestamp(d, {utc: true})} ${pad(d.getUTCHours())}:00`
}
const minute = (unix) => {
  let d = new Date(unix * 1000)
  return `${T.datestamp(d, {utc: true})} ${pad(d.getUTCHours())}:${pad(d.getUTCMinutes())}`
}
const second = (unix) => {
  let d = new Date(unix * 1000)
  return `${T.datestamp(d, {utc: true})} ${pad(d.getUTCHours())}:${pad(d.getUTCMinutes())}:${pad(d.getUTCSeconds())}`
}
...
Sky.Elem.prototype.update({
  colorLabel: function (rgb, name) {
    ...
    return div.attrs({'data-name': name, 'data-rgb': rgb})
  },
